...
def build_json(result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        # Schema 1.4 (additiv): welche Cowork-Overrides angewendet wurden +
        # optionaler Recherche-Log (Quellen/fetched_at) aus der overrides.json —
        # macht die Provenance der recherchierten Werte im Report nachvollziehbar.
        "overrides_applied": result.get("overrides_applied", []),
        "research_log": result.get("research_log", []),
        # Schema 1.5 (additiv): λ-Perturbations-Robustheit (--sensitivity) oder None.
...
def build_markdown(result: dict[str, Any], js: dict[str, Any]) -> str:
    ...
    L.append("")
    L.append("---")
    # Phase 6.3 (additiv): Sensitivity-Robustheit, falls --sensitivity lief.
    sens = js.get("sensitivity") or result.get("sensitivity")
    if sens and sens.get("selections"):
        L.append("## 🧪 Sensitivity (±{:.0f}% λ, {} Szenarien)".format(
            100 * sens.get("rel", 0.15), sens.get("n_scenarios", 9)))
        L.append("")
        L.append("| Selektion | robust | Edge min | Edge max |")
        L.append("|---|---|---|---|")
        for key, v in sens["selections"].items():
            flag = " ⚠️" if key in (sens.get("fragile") or []) else ""
            L.append(f"| {key}{flag} | {v['robust_pct']:.0%} | "
                     f"{v['edge_min_pct']:+.1f}% | {v['edge_max_pct']:+.1f}% |")
        if sens.get("fragile"):
            L.append("")
            L.append("> ⚠️ *fragile* = Edge wechselt unter λ-Perturbation das "
                     "Vorzeichen — Datenartefakt-Verdacht, kein Value.")
        L.append("")

    # Phase 6.3 (additiv): Provenance der Cowork-Recherche, falls vorhanden.
    rlog = js.get("research_log") or result.get("research_log") or []
    if rlog:
        L.append("## 📚 Research-Log (Cowork-Provenance)")
        L.append("")
        L.append("| Slice | Wert | Quelle | fetched_at |")
        L.append("|---|---|---|---|")
        for e in rlog:
            if isinstance(e, dict):
                L.append(f"| {e.get('slice','?')} | {str(e.get('value',''))[:60]} | "
                         f"{str(e.get('url',''))[:50]} | {e.get('fetched_at','')} |")
        L.append("")
