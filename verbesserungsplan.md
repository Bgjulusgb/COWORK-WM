...
## 🧪 Phase 5 — Robustheit gegen Input-Bias (Cowork-Session 2026-06-12)

**Befund aus den Live-Workflows (USA–PAR, CAN–BIH):** Der Bootstrap-p5 misst
nur Sampling-Rauschen, nicht den Bias illustrativer YAML-Inputs. Eine
+34 %-p5-Edge (CAN–BIH Away) kollabierte auf −18 %, sobald die unbelegte
xG-Baseline durch Tor-Proxies ersetzt wurde — der Pick war ein Datenartefakt,
formal aber "p5-fest". Der Divergenz-Guard (schema 1.4) erkennt das Symptom;
Phase 5 misst die Ursache.

### 5.1 Sensitivity-Check (`wm2026/sensitivity.py`, `--sensitivity`) — ✅ erledigt
λ-Perturbations-Grid (±15 % je Seite, 3×3 Szenarien) direkt auf der
**Blend-Matrix** (kein Pipeline-Re-Run): pro bepreister Selektion wird die Edge
in jedem Szenario neu bewertet → `robust_pct` = Anteil Szenarien mit Edge > 0.
`best_value_cons` gilt nur als robust, wenn `robust_pct == 1.0`; sonst Warnung
(hartes Stopp-Signal wie die Divergenz-Warnung). Opt-in (Default off →
Output byte-identisch). Report: additives Feld `sensitivity` (schema 1.5).
Automatisiert exakt den manuellen Tor-Proxy-Sensitivitätslauf.

### 5.2 Edge auf kalibrierten 1X2 (`--edge-calibrated`, opt-in) — ✅ erledigt
Inkonsistenz behoben: Headline-1X2 ist kalibriert (Phase 5), die Edge-Tabelle
rechnete aber auf rohen Modell-p — daher Geister-Edges gegen den eigenen
kalibrierten Konsens. Opt-in ersetzt die 1X2-Inputs der Edge-Rechnung durch die
kalibrierten p; CI-Bänder werden proportional (cal/raw) skaliert. Default off.

### 5.3 `wm2026 research`-Template auf Cowork-v3 — ✅ erledigt
`overrides_template()` scaffoldet zusätzlich `teams.{home,away}`
(elo/fifa_rank/last5_results) + `match.{kickoff_utc,venue}` — das Schema, das
`apply_overrides` seit v3 versteht. Keine Breaking Changes (alte Keys bleiben).

### Verifikation Phase 5
Neue Tests in `tests/test_sensitivity.py` + Erweiterung
`tests/test_overrides_cowork.py`; Invarianten: (a) Null-Perturbation ⇒ Edge
identisch zur Haupttabelle, (b) Default-off ⇒ Report unverändert (bis auf
schema_version), (c) konstruiertes Artefakt ⇒ robust_pct < 1. Volle pinned
Suite + `debug.py` müssen grün bleiben.


## 🔧 Phase 6 — Daten-Contract, Markt-Baseline, Report-Ausgabe (2026-06-12, Session 2)

### 6.1 Orchestrator-Contract gehärtet — ✅ erledigt
`BaseConnector._fetch_with_retry`: die httpx-Client-Konstruktion lag AUSSERHALB
des try — SOCKS-Proxy-Fehler (fehlendes socksio) propagierten am Mock-Fallback
vorbei und hinterließen `error`-Slices mit None-Daten (beobachtet: error ×7 im
CAN–BIH-Report). Jetzt degradiert jeder Client-Init-Fehler sauber zu mock.
Test: `test_client_init_failure_degrades_to_mock_not_error`.

### 6.2 Markt-implizite λ-Baseline (`wm2026/market_baseline.py`, `--baseline market`) — ✅ erledigt
Invertiert vig-freie 1X2- (+ optional O/U-2.5-) Wahrscheinlichkeiten per
Nelder-Mead über der Blend-Matrix zur (λ_home, λ_away) — Ersatz für die
illustrative YAML-xG, wenn recherchierte Quoten vorliegen. `base_xg_source:
"market-implied"` im Result, sane-Range-Guard [0.3, 4.0]. Default `yaml` →
Output unverändert. Caveat dokumentiert: kompoundet mit market_odds-Faktor +
`--calibrate market`. Tests: synthetische Inversion-Recovery (<0.02 Toleranz).

### 6.3 Report-Ausgabe — ✅ erledigt
Markdown rendert jetzt additiv die Sensitivity-Tabelle (robust_pct, fragile-⚠️)
und den Research-Log (Slice/Wert/Quelle/fetched_at) aus der overrides.json.
